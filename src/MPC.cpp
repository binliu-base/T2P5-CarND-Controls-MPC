 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned t  = 0; t < N - 1; t++) {
      ...
      fg[0] += 10*CppAD::pow(vars[a_start + t], 2);             // ok

    }

    // Minimize the value gap between sequential actuations.
    //increased weight on steering changes between adjacent time intervalls is the most important in order to arrive at smooth trajectories.     
    for (unsigned t  = 0; t < N - 2; t++) {
      fg[0] += 600*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);  //ok
...
Solution MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {  
  ...
  for (unsigned i =  delta_start; i < delta_start + latency_idx; i++) {
  ...
  for (unsigned i =  a_start; i < a_start+latency_idx; i++) {
